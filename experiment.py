...
class RememberBest(object):
    ...
    def remember_epoch(self, epochs_df, model, optimizer, subject_id, data_type, model_type, model_number, class_acc):
        """
        ...
        if current_val <= self.lowest_val and current_test_misclass <= self.lowest_test:
            try:
                # th.save(model, 'C:\\Users\\cfcoo\\OneDrive - Ulster University\\Study_2\\results\\nested_results/S{}/{}_{}model{}_{}.pt'.format(subject_id,
                #                                                                                    data_type,
                #                                                                                    model_type, 
                #                                                                                    cropped,
                #                                                                                    model_number))

                th.save(model, f"..//results/S{subject_id}/{model_type}_{data_type}/model_{model_number}.pt")
            except:
                th.save(model, 'C:\\Users\\cfcoo\\Desktop\\Study_2/S{}/{}_{}model_{}.pt'.format(subject_id,
                                                                                                   data_type,
                                                                                                   model_type, 
                                                                                                   model_number))
        ...
        if (current_val == self.lowest_val and current_test_misclass < self.lowest_test):
            self.lowest_test = current_test_misclass
            try:
                # th.save(model, 'C:\\Users\\cfcoo\\OneDrive - Ulster University\\Study_2\\results\\graz\\s{}/{}_{}model{}_{}.pt'.format(subject_id,
                #                                                                                data_type,
                #                                                                                model_type,
                #                                                                                cropped,
                #                                                                                model_number))
...
class Experiment(object):
    ...
    def __init__(self, model, train_set, valid_set, test_set,
                 ...
                 model_number, model_loss_function=None, 
        ...
        if self.log_0_epoch:
            ...
            if remember_best:
                self.rememberer.remember_epoch(self.epochs_df, self.model,
                                               ...
                                               self.model_number, self.class_acc) #added
